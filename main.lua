...
function laserOnCollision(self,event)
    if (event.phase == 'began') then
        ...
        local smallexp = animations.make_small_explosion()
        smallexp:addEventListener('sprite', animations.sprite_listener)
        
        smallexp.x,smallexp.y = self.x,self.y 
        smallexp.isVisible = true
        transition.cancel(self.transitionId)        
        smallexp:play()
        --display.remove(smallexp)
        display.remove(self)
        return true
...
function createLaserShot(idx) 
    ...
    playerLaserShot.name = "Laser shot"..idx
...
--Function which fires the laser
function fireTheLaser()
    --pop laser shot
    local shot = createLaserShot(1)
    shot.x = playerShip.x
    shot.isVisible = true
    local params = {
        time = 800,
...
--load up the current laser shot
--local laserShot = getLaserShot(laserShots)
...
--UI elements for game page: score, timer
